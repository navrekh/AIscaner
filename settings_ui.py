...
class AIScanSettings:
    ...
    # Security Mode
    security_mode_enabled: bool = False
    security_scan_pii: bool = True
    security_scan_credentials: bool = True
    security_scan_macros: bool = True
    security_scan_phishing: bool = True
    security_monitor_exfil: bool = True
    security_block_critical: bool = True
    security_min_severity: str = "HIGH"  # CRITICAL / HIGH / MEDIUM / LOW
...
def show_settings_window(settings_manager, restart_callback=None):
    ...
    # -- TAB 5: Security Mode ---------------------------------------------
    t5 = make_tab("  Security  ")

    section(t5, "SECURITY MODE")
    sec_frame = tk.Frame(t5, bg="#110a0a", padx=16, pady=12)
    sec_frame.pack(fill="x", padx=20, pady=(4, 2))
    tk.Label(sec_frame,
             text="Protect against PII exposure, credential leaks, malicious",
             font=("Helvetica", 9), fg="#888", bg="#110a0a").pack(anchor="w")
    tk.Label(sec_frame,
             text="documents, and data exfiltration attempts.",
             font=("Helvetica", 9), fg="#888", bg="#110a0a").pack(anchor="w", pady=(0,8))

    sec_enabled_var = tk.BooleanVar(value=s.security_mode_enabled)
    sec_toggle = tk.Frame(t5, bg="#1a0a0a", padx=16, pady=10)
    sec_toggle.pack(fill="x", padx=20, pady=2)
    tk.Label(sec_toggle, text="Enable Security Mode",
             font=("Helvetica",10,"bold"), fg="#ff4455",
             bg="#1a0a0a").pack(side="left")
    tk.Checkbutton(sec_toggle, variable=sec_enabled_var,
                   bg="#1a0a0a", activebackground="#1a0a0a",
                   selectcolor="#ff4455").pack(side="right")

    section(t5, "PII DETECTION")
    sec_pii_var   = tk.BooleanVar(value=s.security_scan_pii)
    sec_cred_var  = tk.BooleanVar(value=s.security_scan_credentials)
    check(t5, "Aadhaar, PAN, credit card numbers", sec_pii_var)
    check(t5, "API keys, passwords, tokens, private keys", sec_cred_var)

    section(t5, "DOCUMENT THREATS")
    sec_macro_var    = tk.BooleanVar(value=s.security_scan_macros)
    sec_phish_var    = tk.BooleanVar(value=s.security_scan_phishing)
    sec_exfil_var    = tk.BooleanVar(value=s.security_monitor_exfil)
    sec_block_var    = tk.BooleanVar(value=s.security_block_critical)
    check(t5, "VBA macro detection (Office files)", sec_macro_var)
    check(t5, "Phishing language detection", sec_phish_var)
    check(t5, "PDF JavaScript / Launch action detection", sec_phish_var)

    section(t5, "EXFILTRATION MONITORING")
    check(t5, "Alert on rapid clipboard copying (10+ copies/min)", sec_exfil_var)
    check(t5, "Alert on large clipboard copies (2000+ words)", sec_exfil_var)
    check(t5, "Auto-clear clipboard on CRITICAL PII detection", sec_block_var)

    section(t5, "ALERT THRESHOLD")
    sev_frame = tk.Frame(t5, bg="#111", padx=16, pady=10)
    sev_frame.pack(fill="x", padx=20, pady=2)
    tk.Label(sev_frame, text="Minimum severity to show popup:",
             font=("Helvetica",10), fg="#e8e8f0", bg="#111").pack(anchor="w", pady=(0,6))
    sec_sev_var = tk.StringVar(value=s.security_min_severity)
    for sev, col in [("CRITICAL","#ff2244"),("HIGH","#ff6600"),
                     ("MEDIUM","#ffaa00"),("LOW","#ffdd00")]:
        rf = tk.Frame(sev_frame, bg="#111"); rf.pack(anchor="w")
        tk.Radiobutton(rf, text=sev, variable=sec_sev_var, value=sev,
                       font=("Helvetica",9,"bold"), fg=col, bg="#111",
                       activebackground="#111", selectcolor="#111").pack(side="left")
    ...
    def save_settings():
        ...
        # Security
        settings_manager.settings.security_mode_enabled    = sec_enabled_var.get()
        settings_manager.settings.security_scan_pii        = sec_pii_var.get()
        settings_manager.settings.security_scan_credentials = sec_cred_var.get()
        settings_manager.settings.security_scan_macros     = sec_macro_var.get()
        settings_manager.settings.security_scan_phishing   = sec_phish_var.get()
        settings_manager.settings.security_monitor_exfil   = sec_exfil_var.get()
        settings_manager.settings.security_block_critical  = sec_block_var.get()
        settings_manager.settings.security_min_severity    = sec_sev_var.get()
        # Apply sensitivity preset
        settings_manager.apply_sensitivity(sens_var.get())
        msg_var.set("[OK] Settings saved  -  restart to apply some changes")
        if restart_callback:
