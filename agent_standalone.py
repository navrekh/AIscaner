...
# Security scanner initialized in main() after settings are loaded
_security_scanner = None

def _init_security():
    global _security_scanner
    if not _security_mod:
        return
        ...
        def on_toggle_security(icon, item):
            if _security_scanner:
                ...
                _show_simple_popup("AIScan Security Mode",
                    ...
                    + ("PII, credential and threat detection now running."
                       if _security_scanner.settings.enabled
                       else "PII and threat detection paused."))
            else:
