...
MULTILINGUAL_AI_PHRASES = {
    ...
    "es": [  # Spanish
        "adem?s", "asimismo", "cabe destacar", "en este sentido",
        "es importante se?alar", "en conclusi?n", "por lo tanto",
        "sin embargo", "no obstante", "en definitiva",
        "implementar", "optimizar", "facilitar",
    ],
    "fr": [  # French
        "par ailleurs", "en outre", "il est important de noter",
        "en conclusion", "ainsi", "n?anmoins", "cependant",
        "toutefois", "il convient de", "dans ce contexte",
        "optimiser", "faciliter", "mettre en ?uvre",
    ],
    "ar": [  # Arabic
        "????? ??? ???", "?? ????? ??", "?? ??????",
        "????????", "??? ???", "?? ??? ??????",
...
def detect_language(text: str) -> str:
    ...
    if len(re.findall(r'[\u0600-\u06FF]', text)) > 10:
        return "ar"
    # French indicators
    french_chars = len(re.findall(r'[????????????]', text.lower()))
    if french_chars > 5:
        return "fr"
    # Spanish indicators
    spanish_chars = len(re.findall(r'[?????????]', text.lower()))
    if spanish_chars > 5:
        return "es"
    return "en"
