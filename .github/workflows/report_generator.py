...
def generate_scan_report(
    ...
    rgb(GREY)
    c.setFont("Helvetica", 7)
    c.drawString(20*mm, 4*mm, "Generated by AIScan v6  .  Offline AI Detection  .  aiscan.app")
...
def generate_bulk_report_pdf(summary, output_path: Path) -> Path:
    ...
    for result in (summary.results or [])[:40]:
        if y < 25*mm:
            ...
            rgb(GREY); c.setFont("Helvetica", 7)
            c.drawString(20*mm, 3*mm, "AIScan v6  .  aiscan.app")
            new_page()
    ...
    rgb(BG_MID); c.rect(0, 0, W, 10*mm, fill=1, stroke=0)
    rgb(GREY); c.setFont("Helvetica", 7)
    c.drawString(20*mm, 3*mm, f"AIScan v6  .  aiscan.app  .  Report ID: {int(time.time())}")
